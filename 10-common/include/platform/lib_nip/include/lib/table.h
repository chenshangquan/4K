...
#define l_right  link[1]
  ...
  /* Aggregation. */
  void *aggregate;
#ifdef ZHUZHM_ADD_RIB_NODE_LOCK
  ZLOCK_T rn_lock;
#endif    
};
...
#ifdef ZHUZHM_ADD_RIB_NODE_LOCK
//#warning get interface list lock BEFORE get route lock ,put interface list lock AFTER put route lock,this is just a temporary, should be modified later

#define RNLOCK_WAIT(node) do{\
		if_wait_iflist();\
		ZLOCK_WAIT(&(node->rn_lock));\
	}while(0);

#define RNLOCK_POST(node) do{\
	       ZLOCK_POST(&(node->rn_lock));\
		if_post_iflist();\
	}while(0);
