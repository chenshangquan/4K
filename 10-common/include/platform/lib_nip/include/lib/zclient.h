...
struct zclient
{
  ...
  /* point to the if_table */
  struct if_table *if_table;
  ...
  int (*interface_multicast_change) (int, struct zclient *, zebra_size_t);
#endif
};
...
void zclient_init (struct zclient *, int, struct if_table *);
...
struct zclient_interface *zebra_interface_add_read (struct zclient *);
struct zclient_interface *zebra_interface_state_read (struct zclient *);
struct zclient_connected *zebra_interface_address_add_read (struct zclient *);
struct zclient_connected *zebra_interface_address_delete_read (struct zclient *);

extern int zclient_read (struct zclient *zclient);
extern int zebra_message_send (struct zclient *zclient, int command);
extern int zebra_register_send (int command, ZIPC_ID name, struct zclient *zclient);
