...
#define ZIPC_PATH          "/usr/tmp/"                       /* Prefix of path to bind socket. */
#define ZIPC_ID_LEN        (104 - sizeof(ZIPC_PATH))     /* ZIPC id */
...
inline unsigned long zmsg_get_len(struct zmsg* msg);
inline void zmsg_set_type(struct zmsg* msg, unsigned long type);
inline unsigned long zmsg_get_type(struct zmsg* msg);
