...
*   @version     V5.0_64bit
*   @date        2018-03-05
*   @brief       Cascade messages and struct for H323 protocol 
*   @details     
*/
...
#if ((defined _IOS_PLATFORM_) || (defined _REDHAT_X64_)||(defined _LOONGSON_64_)||(defined _ARM_FT_))
typedef u64    HNODE;
typedef u64    HAPPNODE;
#else
typedef u32    HNODE;
typedef u32    HAPPNODE;
#endif

typedef void (*PCascadeHandler)(s32 nMsgID, u8 *pbyBuffer, u32 dwBufLen, MemId dwUserDatad);

extern void NewConnection(HNODE hListenNode, HNODE hNode, HAPPNODE* phAppNode);
extern void DispatchCascadeMsg(HNODE hNode, HAPPNODE hAppNode, u8 *pBuf, s32 nLen);
...
/**
*媒体类型
*由于历史原因，a_typeEnd、v_typeEnd等范围不准确，请勿在外面使用
...
/**初始化级联库*/
BOOL32 MMcuStackInit(u16 wListenPort,u16 wWatchPortFrom = 0,u16 wWatchPortTo = 0,BOOL32 bSockInit = FALSE);
/**解初始化级联库*/
BOOL32 MMcuStackClean(BOOL32 bSockClean = FALSE);

/**
	* \brief            业务数据进行XML编码
	* \param[in]        nMsgID		:级联消息号（H323MCUMsgType）
	* \param[in]        pbyBuffer   :业务数据内容
	* \param[in]        dwBufLen    :业务数据内容长度
	* \param[in]        pbyXmlBuf   :XML编码结果缓存
	* \param[out]       dwXmlBufLen :XML编码结果缓存长度
	* \retval           XML编码结果长度
*/
...
/**
	* \brief            XML编码解析为业务数据
	* \param[in]        pXmlBuf		  :XML内容缓存
	* \param[in]        nXmlLen       :XML内容长度
	* \param[out]       pResultBuf    :输出结果缓存
	* \param[in]        dwResultBufLen:输出结果缓存长度
	* \retval           业务数据长度
*/
...
public:
	...
	MMCU_API u16  RegisterEventHandler(PCascadeHandler pMsgCallback, MemId dwUserData);
